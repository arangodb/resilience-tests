...
describe('Agency', function () {
  ...
  describe('Agency startup', function() {
    let checkDataLoss = function(agentCount) {
      return instanceManager.startAgency({agencySize: agentCount, agencyWaitForSync: true})
      .then(agents => {
        let data = {'hans': 'wurst'};
        return getLeaderInstance(agents)
        .then(leaderInstance => {
          return writeData(leaderInstance, data)
          .then(() => {
            return leaderInstance;
          });
        })
        .then(leaderInstance => {
          return agencyRequest({
            method: 'POST',
            url: endpointToUrl(leaderInstance.endpoint) + '/_api/agency/read',
            json: true,
            body: [['/']]
          });
        })
        ...
        .then(() => {
          return Promise.all(agents.map(agent => instanceManager.kill(agent)));
        })
        .then(() => {
          return Promise.all(agents.map(agent => instanceManager.restart(agent)));
        })
        .then(() => {
          return getLeaderInstance(agents);
        })
