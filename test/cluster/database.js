...
const expectedSystemCollections = [
  '_analyzers', '_appbundles', '_apps', '_aqlfunctions', '_fishbowl', '_graphs',
  '_jobs', '_queues'
];
...
describe('Database', function() {
  ...
  let debugSetFailAt =
      async function(point, instance) {
    await rp({
      method: 'PUT',
      url: endpointToUrl(instance.endpoint) + '/_admin/debug/failat/' +
  ...
  const checkIfDbExistsOnCoordinator =
      async function(instance, database) {
    db = arangojs({
  ...
  const readDatabaseFromAgency =
      async function(database) {
    for (let i = 0; i < maxAgencyRetries; i++) {
      const [info] = await InstanceManager.rpAgency({
        method: 'POST',
        uri: endpointToUrl(instanceManager.getAgencyEndpoint()) +
            '/_api/agency/read',
        json: true,
        body: [['/']],
      });
  ...
  const waitForDatabaseRemovalFromAgency =
      async function(database) {
    ...
    for (let i = 0; i < maxAgencyRetries; i++) {
      ...
      if (!info.arango.Plan.Databases[database] || info.arango.Plan
                                                      .Databases[database] ===
              undefined) {
        gotRemoved = true;
        return gotRemoved;
  ...
  beforeEach(async function() {
    ...
    db = arangojs(
        {url: instanceManager.getEndpointUrl(), databaseName: '_system'});
  });
  ...
  it('create a database and forcefully reboot the coordinator during creation',
     async function() {
       ...
       await debugSetFailAt(
           'UpgradeTasks::HideDatabaseUntilCreationIsFinished',
           coordinatorToReboot);
       ...
       (async function() {
         try {
           ...
           console.log(new Date().toISOString() + ' Expected error: ' + err);
         }
       })();

       console.log(
           new Date().toISOString() +
           ' Try to read database entry in agency...');

       // check agency
       let found = await readDatabaseFromAgency(databaseName);
       if (!found) {
         expect(true).to.be.false;  // We did not find the entry in the agency,
                                    // something is wrong
       }
       ...
       console.log(
           new Date().toISOString() +
           ' Try to find the database on alive coordinators...');

       // databases are not allowed to show up in the (alive) coordinators
       let checkA = await checkIfDbExistsOnCoordinator(
           coordinatorToVerifyA, databaseName);
       let checkB = await checkIfDbExistsOnCoordinator(
           coordinatorToVerifyB, databaseName);
       expect(checkA).to.be.false;
       expect(checkB).to.be.false;

       let removed = await waitForDatabaseRemovalFromAgency(databaseName);
       // expect that the agency entry got cleaned up via the supervision job
       // (after some time)
       expect(removed).to.be.true;
     });

  it('create a database and kill the coordinator during creation',
     async function() {
       // maintenance job should clean up agency plan
       // make sure to use exactly the same coordinator
       ...
       // we will now kill the coordinator during that wait and have a new db in
       // the plan with the state "isBuilding". This agency plan entry should be
       // cleaned up automatically by the maintenance job afterwards.
       // instanceManager.kill(coordinatorToKill); - not needed anymore, fatal
       // exit internally
       await debugSetFailAt(
           'UpgradeTasks::FatalExitDuringDatabaseCreation', coordinatorToKill);
       ...
       let found = await readDatabaseFromAgency(databaseName);
       if (!found) {
         expect(true).to.be.false;  // We did not found the entry in the agency,
                                    // something is wrong
       }
