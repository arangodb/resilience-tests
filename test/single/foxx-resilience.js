...
async function checkAllServices(services) {
  ...
  await checkServices(services.endpointUrl, resultPerMount, checkServiceAvailable);
  ...
  await checkServices(services.endpointUrl, checksumPerMount, checkBundleChecksum);
...
async function installUtilService(endpoint) {
  await crudServices(endpointToUrl(endpoint),
